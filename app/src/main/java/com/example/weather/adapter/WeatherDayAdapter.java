...
public class WeatherDayAdapter extends RecyclerView.Adapter<WeatherDayAdapter.ForecastViewHolder> {
    private List<DayForecastResponse> forecastDataList;
    private Context context;
    private int selectedPosition = 0;
    private OnItemDayListener listener;

    public WeatherDayAdapter(Context context, List<DayForecastResponse> forecastDataList) {
        this.context = context;
        this.forecastDataList = forecastDataList;
    }

    public void setListener(OnItemDayListener listener) {
    ...
    public void onBindViewHolder(@NonNull ForecastViewHolder holder, int position) {
        DayForecastResponse forecastData = forecastDataList.get(position);
        // TODO: Update rainImg based on weather condition

        if (position == 0) {
            holder.dayTv.setText("Hôm nay");
        } else {
